...
export const SPACING_MEDIUM = 0.5*CARD_WIDTH;   // note: this one has WIDTH, others have HEIGHT
export const SPACING_BIG = 0.5*CARD_HEIGHT;
...
export class CardGameUI extends GameUI {
  ...
  constructor(gameDiv, CoreClass) {
    ...
    for (const card of cardArray) {
      ...
      //for disabled to only have one corner with number and suit
  ...
  _onCardChanged(card) {
    ...
    for (const numberSpan of div.querySelectorAll('.number')) {
      ...
      numberSpan.textContent += card.suit.unicode;
    }
